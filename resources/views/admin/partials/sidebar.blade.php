<div x-data="{ open: false }" @click.away="open = false" class="flex-shrink-0">
    <!-- Mobile Hamburger Button -->
    <button @click="open = true" class="md:hidden p-4">
        <i class="fa fa-bars"></i>
    </button>
    <!-- Sidebar -->
    <div x-show="open || window.innerWidth >= 768" @click.away="open = false"
        class="fixed inset-0 z-50 bg-black bg-opacity-40 md:static md:bg-white md:border-r md:w-64 flex flex-col transition-all"
        :class="{ 'hidden': !open && window.innerWidth < 768, 'block': open || window.innerWidth >= 768 }"
        style="width: 256px;">
        <div class="flex flex-col w-64 border-r border-gray-200 bg-white">
            ...
            <div class="flex flex-col flex-grow px-4 py-4 overflow-y-auto">
                <nav class="flex-1 space-y-2">
                    ...
                    <x-admin.nav-link href="{{ route('admin.sections') }}"
                        :active="request()->routeIs('admin.sections')">
                        <x-icons.section class="w-5 h-5" />
                        <span>Sections</span>
                    </x-admin.nav-link> --}}

                    <x-admin.nav-link href="{{ route('admin.solutions') }}"
                        :active="request()->routeIs('admin.solutions')">
                        <x-icons.solution class="w-5 h-5" />
                        <span>Solutions</span>
                    </x-admin.nav-link>

                    <x-admin.nav-link href="{{ route('admin.features') }}"
                    ...
                    <x-admin.nav-link href="{{ route('admin.contacts') }}"
                        :active="request()->routeIs('admin.contacts')">
                        <x-icons.contact class="w-5 h-5" />
                        <span>Contacts</span>
                    </x-admin.nav-link>

                    <x-admin.nav-link href="{{ route('admin.settings') }}"
